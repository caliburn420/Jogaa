...
**Continuation-only mode** is optional and defaults off. When enabled, compatible providers return only template slot values and new continuation text using their own structured-output transport; Prefill Alchemy restores literal prefix text locally and preserves your selected reasoning settings. Leave it disabled to retain the original fully constrained prefix behavior.
