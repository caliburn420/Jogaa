...
test('Gemini 3 structured prefill requests continuation only at minimum reasoning', async () => {
    extensionSettings.prefillAlchemy.mode = 'on';
    extensionSettings.prefillAlchemy.hide = false;
    const body = {
...
test('Gemini 3 continuation-only output reconstructs during streaming', async () => {
    extensionSettings.prefillAlchemy.mode = 'on';
    extensionSettings.prefillAlchemy.hide = false;
    const body = {
        chat_completion_source: 'makersuite',
    ...
    handlers.get('request-ready')(body);
    const chunks = [
        { candidates: [{ content: { parts: [{ text: '{"continuation":"ta' }] } }] },
        { candidates: [{ content: { parts: [{ text: 'il"}' }] } }] },
    ];
    upstreamFactory = () => new Response(`${chunks.map(chunk => `data: ${JSON.stringify(chunk)}\n\n`).join('')}data: [DONE]\n\n`, {
        headers: { 'content-type': 'text/event-stream' },
    });
    ...
    const text = (await response.text()).split(/\n/)
        .filter(line => line.startsWith('data: {'))
        .map(line => JSON.parse(line.slice(5)).candidates?.[0]?.content?.parts?.[0]?.text ?? '')
        .join('');
    assert.equal(text, 'Prefix: tail');
});
